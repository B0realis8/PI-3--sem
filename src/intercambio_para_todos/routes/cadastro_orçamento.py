            ...
            if isinstance(orc['voo_lista_ida'], list):
                ...
                for v in orc['voo_lista_ida']:
                    if v:
                        ...
                        formatted = (
                            f"Saída: {v.get('cidade_saida_nome', '')} ({v.get('pais_saida_nome', '')}), {v.get('aeroporto_saida', '')}\n"
                            f"{dt_saida}\n\n"
                            f"Chegada: {v.get('cidade_destino_nome', '')} ({v.get('pais_destino_nome', '')}), {v.get('aeroporto_destino', '')}\n"
                            f"{dt_chegada}"
                        )
...
async def content() -> None:
    ...
    column_defs = [
        ...
        {'field': 'pais_nome', 'headerName': 'País', 'sortable': True, 'editable': True},
        {'field': 'cidade_nome', 'headerName': 'Cidade', 'sortable': True, 'editable': True},
    ...
    with ui.page_sticky(position='bottom-right', x_offset=20, y_offset=20).classes('flex items-end gap-3'):
        with ui.column().classes('items-center gap-3'):
            ui.button(icon='add', color='primary', on_click=lambda: render_dialog()).props('fab')
    ...
    async def render_dialog():
        ...
        async def on_filter_pais(e, ida_volta):
                ...
                if ida_volta == 'ida/saida':
                    pais_saida.options = filtered_options
                    pais_saida.update()
                    cidade_saida.value = None
                    cidade_saida.options = []

                elif ida_volta == 'ida/destino':
                    pais_destino.options = filtered_options
                    pais_destino.update()
                    cidade_destino.value = None
                    cidade_destino.options = []

                elif ida_volta == 'volta/saida':    
                    pais_saida_volta.options = filtered_options
                    pais_saida_volta.update()
                    cidade_saida_volta.value = None
                    cidade_saida_volta.options = []

                elif ida_volta == 'volta/destino':
                    pais_destino_volta.options = filtered_options
                    pais_destino_volta.update()
                    cidade_destino_volta.value = None
                    cidade_destino_volta.options = []
        ...
        async def on_filter_cidade(e, ida_volta):
            ...
            if ida_volta == 'ida/saida':
                filtered_options_ida = db_connection.search_database_cidades(search_term, pais_saida.value)
                cidade_saida.options = filtered_options_ida
                cidade_saida.update()

            elif ida_volta == 'ida/destino':
                filtered_options_ida = db_connection.search_database_cidades(search_term, pais_destino.value)
                cidade_destino.options = filtered_options_ida
                cidade_destino.update()

            elif ida_volta == 'volta/saida':
                filtered_options_volta = db_connection.search_database_cidades(search_term, pais_saida_volta.value)
                cidade_saida_volta.options = filtered_options_volta
                cidade_saida_volta.update()
            ...
            #              ─────────Voo─────────────────────────
                    ...
                    with ui.card().classes("w-full"):
                        ...
                        with ui.expansion('Ida', icon='flight').classes('w-full no-wrap'):
                            ...
                            with ui.row().classes('w-full no-wrap'):
                                pais_saida = ui.select({}, label='País de saída',with_input=True).classes('w-1/3 rounded-md').props('outlined dense stack-label').on('filter', lambda e: on_filter_pais(e, 'ida/saida'))
                                cidade_saida = ui.select({}, label='Cidade de saída',with_input=True).classes('w-1/3 rounded-md').props('outlined dense stack-label').on('filter', lambda e: on_filter_cidade(e, 'ida/saida'))
                                aeroporto_saida = ui.input(label='Aeroporto de saída', placeholder='Aeroporto de saída').classes('w-1/3 rounded-md').props('outlined dense stack-label')
                            ...
                            with ui.row().classes('w-full no-wrap'):
                                pais_destino = ui.select({}, label='País de destino',with_input=True).on('filter', lambda e: on_filter_pais(e, 'ida/destino')).classes('w-1/3 rounded-md').props('outlined')
                                cidade_destino = ui.select({}, label='Cidade de destino',with_input=True).on('filter', lambda e: on_filter_cidade(e, 'ida/destino')).classes('w-1/3 rounded-md').props('outlined')
                        ...
                        with ui.expansion('Volta', icon='flight').classes('w-full no-wrap'):
                            ...
                            with ui.row().classes("w-full no-wrap"):
                                ...
                                ui.label('Aeroporto de saída').classes('text-sm font-medium mb-1 w-1/3 align-self-start')
                            with ui.row().classes('w-full no-wrap'):
                                pais_saida_volta = ui.select([], label='País',with_input=True).classes('w-full rounded-md').props('outlined').on('filter', lambda e: on_filter_pais(e, 'volta/saida'))
                                cidade_saida_volta = ui.select([], label='Cidade de saída',with_input=True).classes('w-full rounded-md').props('outlined').on('filter', lambda e: on_filter_cidade(e, 'volta/saida'))
                            ...
                            with ui.row().classes("w-full no-wrap"):
                                ...
                                ui.label('Aeroporto de destino').classes('text-sm font-medium mb-1 w-1/3 align-self-start')
                            with ui.row().classes('w-full no-wrap'):
                                pais_destino_volta = ui.select([], label='País de destino',with_input=True).classes('w-1/3 rounded-md').props('outlined').on('filter', lambda e: on_filter_pais(e, 'volta/destino'))
                                cidade_destino_volta = ui.select([], label='Cidade de destino',with_input=True).classes('w-1/3 rounded-md').props('outlined').on('filter', lambda e: on_filter_cidade(e, 'volta/destino'))
